...
const createUser = async (firstName, lastName, email, profile, age) => {
  ...
  let newUser = { firstName: firstName, lastName: lastName, email: email, profile: profile, age: age, p_childIds: [], n_childIds: [], photoUrl: "", address: "" };
  const userCollection = await users();
  const insertedUser = await userCollection.insertOne(newUser);
  if (!insertedUser.acknowledged || !insertedUser.insertedId) throw { statusCode: 400, message: `Couldn't Create user` };
