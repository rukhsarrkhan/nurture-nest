...
const Home = ({
  ...
  useEffect(() => {
    if (userObjData) {
      if (
        userObjData?.profile === undefined ||
        userObjData?.profile === null ||
        userObjData?.profile === ""
      ) {
      ...
      <Grid item xs={12} sm={7} md={5} lg={4} xl={3} key={child?._id}>
        ...
        >
          ...
          {userObjData?.profile === "PARENT" ? (
            <IconButton
              onClick={() => handleOpen2(child?._id)}
              color="textSecondary"
              aria-label="Delete Child"
            >
              <DeleteIcon
                sx={{ position: "absolute", bottom: 0, right: 150 }}
              />
            </IconButton>
          ) : null}
        </Card>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    deleteChilDAPICall: (childId, obj) =>
      dispatch(deleteChilDAPICall(childId, obj)),
  };
};
