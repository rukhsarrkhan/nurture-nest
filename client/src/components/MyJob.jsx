...
useEffect(() => {
  ...
  if (loading) {
    ...
    <Button  onClick={() => {navigate(-1)}} variant="filled" sx={{bgcolor:purple[700]}}>Back</Button><br/><br/>
      ...
      <CardContent>
        ...
        <Typography color="text.secondary" paragraph sx={{paddingLeft:"5px"}}>
          {showData?.city}
        </Typography>

        <Typography color="text.primary" paragraph sx={{paddingLeft:"15px"}}>
          State:
        </Typography>
        <Typography color="text.secondary" paragraph sx={{paddingLeft:"5px"}}>
          {showData?.state}
      ...
      <Typography color="text.secondary" paragraph>{shiftDays}</Typography>
        ...
        <div style={{ display: "flex" }}>
        <Typography color="text.primary" paragraph>Salary:</Typography>
        <Typography color="text.secondary" paragraph  sx={{paddingLeft:"7px"}}>
  ...
  <Button
    variant="contained"
    onClick={() => {
      navigate("/job/allApplicantions/1", { state: { jobId:showData._id } });
    }}
