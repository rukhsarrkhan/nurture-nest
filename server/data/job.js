...
const removeJob = async (jobId) => {
    ...
    if (jobInDB.nannyId != null)
        throw { statusCode: 400, message: "Job Cannot be deleted. Please fire the nanny assigned to this job first to delete this job" };
    const jobCollection = await jobs();
...
const getAllApplicants = async (jobId) => {
    const jobCollection = await jobs();
    let allApplications = await jobCollection.findOne({ _id: ObjectId(jobId) }, { projection: { _id: 0, applications: 1 } });
...
const addApplication = async (
    ...
    if (applicationCreated.modifiedCount === 0) {
        throw "could not create application successfully";
    }
...
const getApplication = async (jobId, applicationId) => {
    ...
    let applicationFound = await movieCollection.findOne(
        { _id: ObjectId(jobId), applications: { $elemMatch: { _id: ObjectId(applicationId) } } },
        {
            projection: {
                _id: 0,
