...
router.route("/createJob/:parentId/:childId").post(async (req, res) => {
  ...
  try {
    ...
    const updateChild = await jobCollection.assignJobToChild(
      childId,
      jobCreated._id.toString()
    );
...
router
  ...
  .get(async (req, res) => {
    ...
    try {
      const searchedApplicants = await jobCollection.searchApplications(
        jobId,
        searchTerm,
        pageNum
      );
      if (!searchedApplicants) {throw { statusCode: 400, message:"Couldn't get applications"}}
      console.log(searchedApplicants.allApplications[0].applications[0])
...
router.route("/setNanny/:jobId/:nannyId").post(async (req, res) => {
  ...
  try {
    ...
    const child = await childCollection.getChildById(
