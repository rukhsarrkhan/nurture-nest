...
const CreateJobModal = (props) => {
    ...
    const handleSubmit = async (event) => {
        ...
        let shiftFromChec = await validation(timeFrom, helpers.isDateValid(timeFrom, "Shift Timing from"));
        if (shiftFromChec !== "") {
            setShiftError(true);
            setErrorText(shiftFromChec);
            return;
        }
        let shiftToChec = await validation(timeTo, helpers.isDateValid(timeTo, "Shift Timing to"));
        if (shiftToChec !== "") {
            setShiftError(true);
        ...
        if (shift[0] && shift[1] && specialCare && description && address && state && zipCode && salary) {
            const data = {
                    ...
                    >
                        ...
                        >
                            ...
                            <FormControl
                                ...
                                variant="standard"
                            >
                                <FormLabel component="legend" color="secondary">Pick Shift Days</FormLabel>
                                <FormGroup>
                                    <FormControlLabel
                                        control={
                                            <Checkbox
