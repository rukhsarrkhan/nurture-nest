...
const ApplyToJobModal = (props) => {
    ...
    const handleSubmit = async (event) => {
        ...
        if(shift){setShift(shift.trim())}
        if(description){setDescription(description.trim())}
        if(disability){setDisability(disability.trim())}
        if(experience){setExperience(experience.trim())}
        if(coverLetter){setCoverLetter(coverLetter.trim())}
