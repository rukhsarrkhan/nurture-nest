...
const ApplyToJobModal = (props) => {
  ...
  const handleSubmit = async (event) => {
    ...
    let nannyNameChec = await validation(
      nannyName,
      helpers.isNameValid(nannyName, "Nanny Name")
    );
    if (nannyNameChec !== "") {
      setNannyName(true);
    ...
    let contactChec = await validation(
      contact,
      helpers.validatePhoneNumber(contact, "Phone Number")
    );
    if (contactChec !== "") {
    ...
    let cityChec = await validation(
      city,
      helpers.isCityParentValid(city, "City")
    );
    if (cityChec !== "") {
      setCityError(true);
    ...
    let distanceChec = await validation(
      distance,
      helpers.isDistanceInputValid(distance, "Distance from Job")
    );
    if (distanceChec !== "") {
