...
const assignJobToChild = async (childId, jobId) => {
    ...
    const updatedChild = await childCollection.updateOne({ _id: ObjectId(childId) }, { $set: { jobId: ObjectId(jobId) } });
...
const removeJobFromChild = async (childId) => {
    ...
    const childCollection = await childs();
    const updatedChild = await childCollection.updateOne({ _id: ObjectId(childId) }, { $set: { jobId: null } });
    if (!updatedChild.acknowledged || updatedChild.modifiedCount == 0)
        throw {
            statusCode: 400,
            message: "Couldn't update child after creating Job",
...
const getJobByNannyId = async (nannyId) => {
    ...
    if (!ObjectId.isValid(nannyId)) throw { statusCode: 400, message: "invalid object ID" };
    const jobCollection = await jobs();
...
const getJobById = async (jobId) => {
    ...
    if (jobFound === null) throw { statusCode: 400, message: "No job with that id" };
...
const removeJob = async (jobId) => {
    ...
    if (jobInDB.nannyId != null)
        throw {
            statusCode: 400,
            message: "Job Cannot be deleted. Please fire the nanny assigned to this job first to remove this child",
        };
    const jobCollection = await jobs();
...
const addApplication = async (
    ...
    const jobCollection = await jobs();
    const applicationCreated = await jobCollection.updateOne({ _id: ObjectId(jobId) }, { $push: { applications: newApplication } });
...
const setNannytoJob = async (jobId, nannyId) => {
    ...
    let jobUpdated = await jobCollection.updateOne({ _id: ObjectId(jobId) }, { $set: { nannyId: ObjectId(nannyId) } });
...
const findMyAppliedJobs = async (nannyId) => {
    ...
    let nanniesFound = await jobCollection
        .aggregate([{ $unwind: "$applications" }, { $match: { "applications.nannyId": ObjectId(nannyId) } }])
        .toArray();
...
const getJobByChildId = async (childId) => {
    ...
    if (jobFound !== null) {
        ...
        let nannyId = jobFound.nannyId !== null ? jobFound.nannyId.toString() : null;
        if (nannyId !== null) {
