...
export const getallJobsAPICall = (nannyId, pageNum) => {
    return async (dispatch) => {
        try {
            let resp = await axios.get(`http://localhost:3000/job/getJobs/AllJobs/${nannyId}/${pageNum}`);
...
export const searchJobsAPICall = (nannyId, searchTerm, pageNum) => {
    return async (dispatch) => {
        try {
            let resp = await axios.get(`http://localhost:3000/job/searchJobs/${nannyId}/${searchTerm}/${pageNum}`);
